...
type ToastType = "success" | "error" | "info" | "edit" | "delete";
type ToastCategory = "calendar" | "notes" | "tasks";

interface StandardizedToastProps {
  title: string;
  description?: string;
  type?: ToastType;
...
const categoryIconMap = {
  ...
  notes: <FileText className="h-4 w-4 text-blue-600" />,
  tasks: <CheckSquare className="h-4 w-4 text-green-600" />,
};
...
export const showStandardizedToast = ({
  title,
  description,
  type = "success",
  category,
  duration = 4000,
}: Omit<StandardizedToastProps, "onDismiss">) => {
  return toast.custom(
    (toastId) => (
      <StandardizedToast
        title={title}
...
export default StandardizedToast;
